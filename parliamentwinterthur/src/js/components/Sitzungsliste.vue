    ...
    <div v-else>
      ...
      >
        <div class="pw-sitzung-kopf" @click="toggleSitzung(sitzung.id)">
          ...
          <a v-if="sitzung.url" :href="sitzung.url" target="_blank" @click.stop class="pw-extern-link">↗</a>
        ...
        <div v-if="offeneSitzungen.includes(sitzung.id)" class="pw-sitzung-details">
          ...
          <div class="pw-sitzung-bemerkungen">
            ...
            />
            <button class="pw-btn-klein" @click="speichereSitzungBemerkungen(sitzung)">Speichern</button>
          </div>
              ...
              <div class="pw-traktandum-felder">
                ...
                <div class="pw-traktandum-notizen">
                  <div v-for="(n, idx) in parseTraktandumNotizen(t.id)" :key="idx" class="pw-notiz-klein">
                    ...
                    {{ n.text }}
                    <button class="pw-btn-mini" @click="loesche_traktandum_notiz(t.id, idx)">✕</button>
                  </div>
                  <div class="pw-neue-notiz-klein">
                    ...
                    />
                    <button class="pw-btn-mini" @click="fuegeNotizHinzu(t.id)">+</button>
                  </div>
                </div>
                <button class="pw-btn-klein" @click="speichereTraktandumFelder(t)">Speichern</button>
              </div>
            </div>
...
export default {
  ...
  methods: {
    async ladeSitzungen() {
      this.laden = true
      try {
        const { data } = await axios.get(generateUrl('/apps/parliamentwinterthur/sitzungen'), {
          params: { limit: 100 },
        })
        this.sitzungen = data
        data.forEach(s => {
    ...
    async ladeTraktandenFuerSitzung(sitzungId) {
      if (this.traktanden[sitzungId]) return
      this.ladenTraktanden = { ...this.ladenTraktanden, [sitzungId]: true }
      try {
        const { data } = await axios.get(
          generateUrl(`/apps/parliamentwinterthur/sitzungen/${sitzungId}/traktanden`)
        )
    ...
    async speichereSitzungBemerkungen(sitzung) {
      try {
        await axios.put(
          generateUrl(`/apps/parliamentwinterthur/sitzungen/${sitzung.id}`),
    ...
    async speichereTraktandumFelder(traktandum) {
      try {
        ...
        await axios.put(
          generateUrl(`/apps/parliamentwinterthur/sitzungen/${sitzungId}/traktanden/${traktandum.id}`),
          {
