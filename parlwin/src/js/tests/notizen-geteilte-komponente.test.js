...
describe('Notizen — eine geteilte Komponente für Geschäft und Vorstoss', () => {
  ...
  // Die gelöschten Notizen leben jetzt DIREKT in NotizenListe (nicht mehr in der
  // Aktionszeitleiste): als Vermerk «… hat seine Notiz gelöscht», mit einem
  ...
  it('rendert eine gelöschte Notiz als Vermerk mit Wiederherstellen nur für den Autor', () => {
    ...
    const eigen = shallowMount(NotizenListe, {
    ...
    const fremd = shallowMount(NotizenListe, {
      props: { basisUrl: 'geschaefte/1', notizen: [geloescht], aktuelleUid: 'jemandanderes' },
    })
