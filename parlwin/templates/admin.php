...
<div class="section" id="parlwin-admin-settings">
    <h2><?php p($l->t('Parlament Winterthur – Einstellungen')); ?></h2>
    ...
    <form id="pw-settings-form" class="pw-admin-form">
        <section class="pw-admin-card">
            ...
            <div class="pw-admin-grid">
                ...
                <div class="pw-admin-field">
                    ...
                    <input
                        type="text"
                        id="pw-nextcloud-gruppe"
                        name="nextcloud_gruppe"
                        class="pw-input"
                        list="pw-nextcloud-gruppen"
        ...
        <section class="pw-admin-card">
            ...
            <div class="pw-mitglied-toolbar">
                ...
                <button type="button" id="pw-btn-members-save" class="button">
                    <?php p($l->t('Zuordnungen speichern')); ?>
                </button>
        ...
        <section class="pw-admin-card">
            ...
            <div class="pw-admin-grid">
                <div class="pw-admin-field">
                    ...
                    <input
                        type="text"
                        id="pw-kalender-nutzer"
                        name="kalender_nutzer"
                        class="pw-input"
                        list="pw-kalender-nutzer-liste"
                        value="<?php p($_['kalender_nutzer']); ?>"
                        placeholder="z.B. admin"
                    />
                    <datalist id="pw-kalender-nutzer-liste">
                        <?php foreach ($kalenderAktiv as $user): ?>
                            <option value="<?php p((string) ($user['uid'] ?? '')); ?>" label="<?php p((string) ($user['label'] ?? '')); ?>"></option>
                        <?php endforeach; ?>
                        <?php foreach ($kalenderInaktiv as $user): ?>
                            <option value="<?php p((string) ($user['uid'] ?? '')); ?>" label="<?php p((string) ($user['label'] ?? '')); ?> [inaktiv]"></option>
                        <?php endforeach; ?>
                    </datalist>
...
(function() {
    ...
    const btnMembersSave = document.getElementById('pw-btn-members-save');
    ...
    form.addEventListener('submit', function(e) {
        ...
        const data = {};
        formData.forEach((v, k) => data[k] = String(v || '').trim());

        fetch(OC.generateUrl('/apps/parlwin/settings'), {
            method: 'POST',
            headers: authHeaders({
                'Content-Type': 'application/json',
            }),
        ...
        .then(() => {
            updateGroupState();
            updateKalenderState();
            OC.Notification.showTemporary(t('parlwin', 'Einstellungen gespeichert'));
        })
        .catch(err => {
    ...
    btnMembersSave?.addEventListener('click', saveMemberMappings);
